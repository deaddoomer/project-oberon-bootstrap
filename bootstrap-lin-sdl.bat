...
"%OBERON_EXE%" Link.ob07 win32con -out LinkELF.exe -upper -nochk bc -stk 1 -def LinkTargetELF
if %errorlevel% neq 0 exit /b %errorlevel%

"%OBERON_EXE%" Link.ob07 win32con -out LinkELFDYN.exe -upper -nochk bc -stk 1 -def LinkTargetELFDYN
if %errorlevel% neq 0 exit /b %errorlevel%
...
..\AsciiToText.exe ^
  ...
  ..\project-oberon\LINUX386.Build.Tool.txt    Build.Tool ^
  ...
  ..\project-oberon\LINUX386.Libraries.Mod.txt Libraries.Mod ^
...
copy /z ..\project-oberon\LINUX386FB.readme.txt readme.txt
if %errorlevel% neq 0 exit /b %errorlevel%
...
..\Compile.exe ^
  ...
  Libraries.Mod Host.Mod Input.Mod Display.Mod ^
